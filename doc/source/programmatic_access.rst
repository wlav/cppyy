...
Binding developers typically required programmatic access to ``cppyy``.
Programmatic access to the supported features has two aspects, one is a command
line interface and one intended to support automated generation of an end-user
...
This provides base-level access to ``cling``::
...
These issues are addressed by a blend of Python packaging and CMake. CMake
provides:
